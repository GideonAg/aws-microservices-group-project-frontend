...
export default function TaskDetail() {
  ...
  const fetchTask = async () => {
    try {
      ...
      const formattedDeadline = format(new Date(res.data.deadline), "yyyy-MM-dd'T'HH:mm");
    ...
    <div className="p-6 max-w-3xl mx-auto bg-white shadow rounded-lg">
          ...
          <>
            ...
            <div className="sm:col-span-2 mt-2">
              <button
                ...
                disabled={isUpdating}
                className={`px-4 py-2 rounded text-white ${isUpdating ? "bg-[#FF5A00]/80" : "bg-[#FF5A00] hover:bg-[#FF5A00]/80"}`}
              >
                {isUpdating ? (
      ...
      {task.status === "closed" && userComment && (
        <div className="mt-6 bg-gray-100 p-4 rounded">
          <h2 className="font-semibold text-gray-700 mb-2">💬 User's Completion Comment:</h2>
